...
from camcontrast.resnet import RESNET_CONSTRUCTORS
...
class Up(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x = torch.cat([x1, x2], dim=1) if self.in_channels_2 > 0 else x1
...
class UNetEncoder(nn.Module):
    def __init__(self, n_channels):
        super().__init__()
        self.down0 = DoubleConv(n_channels, 64)
    ...
    def forward(self, x):
        for down in [self.down0, self.down1, self.down2, self.down3, self.down4]:
            x = down(x)
...
class UNet(nn.Module):
    def __init__(self, n_channels, bilinear=True, encoder_backbone='u-net-encoder', imagenet_init=True):
        """
        Args:
        ...
        if encoder_backbone == 'u-net-encoder':
            unet_encoder = UNetEncoder(n_channels)
            self.encoder = nn.ModuleDict({
                'down0': unet_encoder.down0,
                'down1': unet_encoder.down1,
                'down2': unet_encoder.down2,
                'down3': unet_encoder.down3,
                'down4': unet_encoder.down4,
            })
