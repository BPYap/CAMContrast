...
from camcontrast.config import logger, is_distributed, is_main_process
from camcontrast.transforms_utils import TransformsDataset, DEFAULT_TRANSFORM


def _load_model_state(model, model_path, strict=True, map_unet=False):
    logger.info(f"Loading model parameters from '{model_path}'")
    _loaded_states = torch.load(model_path, map_location='cpu')
    if map_unet:
        mappings = {
            "encoder.conv1.weight": "encoder.down1.0.weight",
            "encoder.bn1.weight": "encoder.down1.1.weight",
            "encoder.bn1.bias": "encoder.down1.1.bias",
            "encoder.bn1.running_mean": "encoder.down1.1.running_mean",
            "encoder.bn1.running_var": "encoder.down1.1.running_var",
            "encoder.layer1.": "encoder.down2.1.",
            "encoder.layer2.": "encoder.down3.",
            "encoder.layer3.": "encoder.down4.",
            "encoder.layer4.": "encoder.down5."
        }
        loaded_states = dict()
        for key, param in _loaded_states.items():
            for old_prefix, new_prefix in mappings.items():
                if key.startswith(old_prefix):
                    loaded_states[key.replace(old_prefix, new_prefix)] = param
    else:
...
class JobRunner:
    def __init__(self, args, model, callback):
        ...
        if args.do_eval and not args.do_train and os.path.exists(args.model_dir):
            ...
            map_unet = hasattr(args, 'segmentation_architecture') and args.segmentation_architecture == 'u-net' and \
                       args.encoder_backbone.startswith('resnet')
            _load_model_state(self.model, args.pretrain_model_path, strict=False, map_unet=map_unet)
    ...
    def train_model(self, train_data_loader, eval_data_loader=None):
        ...
        if args.checkpoint_dir and os.path.exists(args.checkpoint_dir) and args.checkpoint_dir.split("-")[
            -1].isdigit():
